  ...
  if (typeof(self.on) !== 'undefined') {
    ...
    self.on('message', function(msgEvent) {
      switch (msgEvent.name) {
        ...
        default:
            if( msgEvent.message && msgEvent.message.callbackID && instance.callbackQueue.callbacks[msgEvent.message.callbackID]) {
                instance.callbackQueue.callbacks[msgEvent.message.callbackID](msgEvent.message.value);
            }
          break;
      }
    });
    ...
    instance.safariMessageHandler = function(msgEvent) {
      switch (msgEvent.name) {
        ...
        case 'contextMenu.click':
          ...
          break;
        default:
          if( msgEvent.message.callbackID && instance.callbackQueue.callbacks[msgEvent.message.callbackID]) {
            instance.callbackQueue.callbacks[msgEvent.message.callbackID](msgEvent.message.value);
          }
          break;
      }
  ...
  switch(instance.detectedBrowser) {
    ...
    case 'Safari':
      instance.bgMessage = function(thisJSON, callback) {
        ...
        // check if we are a background script
        if(safari.application) {
            safari.extension.globalPage.contentWindow.postMessage(thisJSON,safari.extension.globalPage.contentWindow.location.origin);
        }else {
