...
function searchNames(bodytext, sendResponse, tabId) {
    if( names == false ){
        console.log('Update in progress, adding to callback list')
        callbackQ.push(function(){
            searchNames(bodytext, sendResponse, tabId);
        });
        return true;
    }
    else sendResponse(do_search(bodytext,tabId));
}
...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        switch(request.requestType) {
            case 'searchNames': searchNames(request.bodytext, sendResponse, sender.tab.id);
                             break;
