...
function on_vendor_whitelist( sender ) {
    var hostname = parseURL(sender.url).hostname;
    return _.indexOf( vendor_whitelist, hostname) >=0;
}
...
function searchNames(bodytext, sendResponse, senderTab) {
    if(  update_pending ){
        ...
        var personal_whitelisted = on_whitelist(senderTab);
        var vendor_whitelisted = on_vendor_whitelist(senderTab);
        var blacklisted = on_blacklist(senderTab);
        if( (personal_whitelisted || vendor_whitelisted) && !blacklisted ) {
            ...
            tabData.set(senderTab.id, { disabled:true,
                                        can_enable: !on_blacklist(senderTab) } );
            sendResponse([]);
        }
    }
}
...
function addWhitelist( hostname, sendResponse ) {
    ...
    if( SAFARI ) {
        safari.extension.settings['whitelist'] = whitelist.join(',');
