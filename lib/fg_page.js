...
function generateTooltip(id, callback) {
    ...
    var max_list_height = windowHeight - (100*2); // Liste darf maximal so hoch wie das Browserfenster
    ...
    BabelExt.bgMessage({requestType:'detail_for_id',id:id},function(person){
            ...
            tt_content = '<div class="lobbyradar_top"><div class="header-person">';
            tt_content += person.names[0];
            tt_content += '</div>';
            tt_content += 'Für <strong><a href="';
            tt_content += detail_url_extern+id;
            tt_content += '" target="_blank" class="name_info">';
            tt_content += person.names[0];
            tt_content += '</a></strong> wurden die folgenden Verbindungen gefunden:</div><section class="lobbyradar_middle">';
